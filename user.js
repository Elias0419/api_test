...
function localStoreGet() { // get userID from local storage if it exists
	if (!running) {
		return;
	}
	return new Promise((resolve, reject) => {
		...
		if (userID) {
			...
			getUserID().then(id => { //otherwise get it from accountFrame
				resolve(id);
			}).catch(error => {
	...
	try {
		...
		return new Promise((resolve, reject) => {
			setTimeout(() => {
				...
				let maxPages1 = topPage[12].innerText; //TODO handle cases of less than 12 pages
...
async function getPages(token, maxPages1) { //main loop
	//console.log("token "+token+" maxPages1 "+maxPages1+" currentPage "+currentPage)
	if (!startTime) {
		startTime = new Date();
...
function direWarning() {
	if (!localStorage.getItem("warningRead")) { // first run only
